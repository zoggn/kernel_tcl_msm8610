...
{
        wdog_reset_flag=1;
	 pr_info("JCH:Founded tz dump,bootinfo We should set power up reason to wdog reset.\n");
}
u32 bi_powerup_reason(void)
{
	...
	if(PU_REASON_SW_AP_RESET==reason && wdog_reset_flag==1)
		return PU_REASON_WDOG_AP_RESET;
	return reason;
}

void char_powerup_reason( char *pwr_on_reason)
{
       switch(bi_powerup_reason()) {
		case PU_REASON_USB_CABLE:
			strcat(pwr_on_reason, "USB_CABLE");
			break;
		case PU_REASON_FACTORY_CABLE:
			strcat(pwr_on_reason, "FACTORY_CABLE");
			break;
		case PU_REASON_PWR_KEY_PRESS:
			strcat(pwr_on_reason, "PWR_KEY_PRESS");
			break;
		case PU_REASON_CHARGER:
			strcat(pwr_on_reason, "USB_CHARGER");
			break;
		case PU_REASON_POWER_CUT:
			strcat(pwr_on_reason, "POWER_CUT");
			break;
		case PU_REASON_SW_AP_RESET:
			strcat(pwr_on_reason, "SW_AP_RESET");
			break;
		case PU_REASON_WDOG_AP_RESET:
			strcat(pwr_on_reason, "WDOG_AP_RESET");
			break;
		case PU_REASON_AP_KERNEL_PANIC:
			strcat(pwr_on_reason, "AP_KERNEL_PANIC");
			break;
		case PU_REASON_INVALID:
			strcat(pwr_on_reason, "REASON_INVALID");
			break;
		default:
			break;
	}
...
*/
static void bootinfo_print_bl(struct bl_build_sig *bl)
{
	int i;
	pr_info("JCH:bootinfo_print_bl:bl_build_sig_count=%d\n",bl_build_sig_count);
	for (i = 0; i < bl_build_sig_count; i++) {
		bl[i].item[MAX_BLD_SIG_ITEM - 1] = 0;
		bl[i].value[MAX_BLD_SIG_VALUE - 1] = 0;
		pr_info("JCH:bl[%d] (item)%s=(value)%s \n ",i,bl[i].item,bl[i].value);
	}
}
...
static int __init bootinfo_init_module(void)
{
	...
	bootinfo_print_bl(bl_build_sigs);
