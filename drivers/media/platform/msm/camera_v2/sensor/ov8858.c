...
static uint32_t lenc_start_addr = 0, lenc_end_addr = 0, wb_start_addr = 0, wb_end_addr = 0;
#endif //OTP_FEATURE
	...
	struct ov8858_otp_struct *current_otp)
{
	...
	if(rc) {
		lenc_start_addr = start_addr;
		lenc_end_addr = end_addr;
		OV8858_write_i2c(s_ctrl, 0x3d84, 0xc0); //enable OTP read
	...
	struct ov8858_otp_struct *current_otp)
{
	...
	if(rc) {
		wb_start_addr = start_addr;
		wb_end_addr = end_addr;
		OV8858_write_i2c(s_ctrl, 0x3d84, 0xc0); //enable OTP read
		OV8858_write_i2c(s_ctrl, 0x3d88, (start_addr >> 8) & 0xff); //start address
		OV8858_write_i2c(s_ctrl, 0x3d89, start_addr & 0xff);
		OV8858_write_i2c(s_ctrl, 0x3d8A, (end_addr >> 8) & 0xff); //end address
...
static int32_t ov8858_otp_clearbuff_config(struct msm_sensor_ctrl_t *s_ctrl,
	void __user *argp)
{
	uint32_t i = 0;
	
	CDBG("enter, lenc:0x%x-0x%x, wb:0x%x-0x%x\n", lenc_start_addr, lenc_end_addr, wb_start_addr, wb_end_addr);
	mutex_lock(s_ctrl->msm_sensor_mutex);
	if (lenc_start_addr && lenc_end_addr) {
		for(i = lenc_start_addr; i <= lenc_end_addr; i++) { //clear OTP buffer
			OV8858_write_i2c(s_ctrl, i, 0);
		}
		lenc_start_addr = 0;
		lenc_end_addr = 0;
	}
	if (wb_start_addr && wb_end_addr) {
		for(i = wb_start_addr; i <= wb_end_addr; i++) { //clear OTP buffer
			OV8858_write_i2c(s_ctrl, i, 0);
		}
		wb_start_addr = 0;
		wb_end_addr = 0;
...
static struct msm_sensor_fn_t ov8858_sensor_func_tbl = {
	...
	.otp_clearbuff_config = ov8858_otp_clearbuff_config,
#endif //OTP_FEATURE
};
