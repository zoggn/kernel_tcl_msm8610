...
///////////////////////////////////////////////////////
static void show_jrd_map_vma(struct seq_file *m, struct vm_area_struct *vma, int is_pid)
{
        struct mm_struct *mm = vma->vm_mm;
        struct file *file = vma->vm_file;
        struct proc_maps_private *priv = m->private;
...
static int show_jrd_map(struct seq_file *m, void *v, int is_pid)
{
        struct vm_area_struct *vma = v;
        struct proc_maps_private *priv = m->private;
        struct task_struct *task = priv->task;
        show_jrd_map_vma(m, vma, is_pid);
        if (m->count < m->size)  /* vma is copied successfully */
                m->version = (vma != get_gate_vma(task->mm))
                        ? vma->vm_start : 0;
        return 0;
}
static int show_pid_jrd_map(struct seq_file *m, void *v)
{
        return show_jrd_map(m, v, 1);
}
static const struct seq_operations proc_pid_jrd_maps_op = {
        .start  = m_start,
        .next   = m_next,
        .stop   = m_stop,
        .show   = show_pid_jrd_map
};
static int pid_jrd_maps_open(struct inode *inode, struct file *file)
{
        return do_maps_open(inode, file, &proc_pid_jrd_maps_op);
}
const struct file_operations proc_pid_jrd_maps_operations = {
        .open           = pid_jrd_maps_open,
        .read           = seq_read,
        .llseek         = seq_lseek,
